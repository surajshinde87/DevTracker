...
const Navbar = () => {
  ...
  useEffect(() => {
    if (mobileMenuOpen) {
      ...
      if (mobileMenuRef.current) {
        const innerMenu =
          mobileMenuRef.current.querySelector(".mobile-menu-inner");
        if (innerMenu) {
          const childrenArray = Array.from(innerMenu.children);
          gsap.from(childrenArray, {
            opacity: 0,
            y: 10,
            duration: 0.4,
            stagger: 0.05,
  ...
  useEffect(() => {
    const handleScrollHighlight = () => {
      ...
      const sections = [
        ...
        {
          id: "#why-devtracker",
          element: document.querySelector("#why-devtracker"),
        },
      ];
  ...
  return (
    <>
      ...
      >
        <div className="mobile-menu-inner flex flex-col space-y-6">
          {navItems.map((item, index) => (
            <Link
              key={index}
              href={item.path}
